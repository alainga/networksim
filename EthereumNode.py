...
class EthereumNode(EthereumConstant):
    id: int
    ############################
    # initialization
    ############################
    def __init__(self, node_id: int, DNS_rnd: dict , t: float, hard_coded_dns, DG: object, MAX_OUTBOUND_CONNECTIONS: int = 13,
                 ...
                 is_evil: bool = False, connection_strategy: str = 'stand_eth', lite=False) -> object:
    ...
    def update_outbound_connections(self, t):  # node asks for connection
        if len(self.outbound) >= self.MAX_OUTBOUND_CONNECTIONS or len(self.inbound) + len(self.outbound) >= self.MAX_TOTAL_CONNECTIONS:
            raise ValueError("illegal outbound!")
        ...
        if (self.connection_strategy == 'p2c_max') or (self.connection_strategy == 'p2c_min'):
            ...
            if (address is not None) and (address2 is not None):
                ...
                if addresses_in_graph:
                    ...
                    if self.connection_strategy == 'p2c_min':
                        address = address if degree1 <= degree2 else address2
                    elif self.connection_strategy == 'p2c_max':
                        address = address if degree1 >= degree2 else address2
        elif self.connection_strategy == 'geo_bc':
            numb_bubble = 5
    ...
    def ask_for_outbound_connection(self, t, address):  # node is asked to establish connection
        #if len(self.inbound) >= self.MAX_TOTAL_CONNECTIONS - self.MAX_OUTBOUND_CONNECTIONS:  # before
        if len(self.inbound) + len(self.outbound) >= self.MAX_TOTAL_CONNECTIONS:
            #print("node ", self.id, " rejected node ", address, " because of too many total connections")
            return False
        if len(self.inbound) >= 17:  # see maxInboundConns() go-ethereum/p2p/server.go
            #print("node ", self.id, " rejected node ", address, " because of too many inbound connections")
            return False
        if address in list(self.outbound.keys()):
            return False
    ...
    def receive_message(self, t, envelope):
        ...
        if envelope['connect_as_outbound'] == 'accepted':
            if len(self.outbound) + len(self.inbound) >= self.MAX_TOTAL_CONNECTIONS:
                raise ValueError("More peers than allowed! receiver: ", envelope['receiver'], ", sender: ", sender_node_id)
    ...
    def interval_processes(self, t):
        ...
        if len(self.outbound) > self.MAX_OUTBOUND_CONNECTIONS:  #previously >=, makes a huge difference
            #raise ValueError("outbound was > max outbound")
        ...
        if len(self.inbound) + len(self.outbound) < self.MAX_TOTAL_CONNECTIONS and \
                len(self.outbound) < self.MAX_OUTBOUND_CONNECTIONS:  #before: len(self.outbound) < self.MAX_OUTBOUND_CONNECTIONS
            neighbours_to_connect = min(self.MAX_OUTBOUND_CONNECTIONS - len(self.outbound), \
                                        self.MAX_TOTAL_CONNECTIONS - len(self.inbound) - len(self.outbound))  # previously: if self.MAX_OUTBOUND_CONNECTIONS - 5 < len(list(self.outbound)) else 5
            for _ in range(neighbours_to_connect):
                ...
                if envelope4 is not None:
                    ...
                    #print("envelope4 was none in interval_process")
                    return None
    ...
    def connect_lite(self, t):
        self.output_envelope = []  # maybe problematic
        output = []
        if len(self.inbound) + len(self.outbound) < self.MAX_TOTAL_CONNECTIONS and \
                len(self.outbound) < self.MAX_OUTBOUND_CONNECTIONS:  #before: len(self.outbound) < self.MAX_OUTBOUND_CONNECTIONS
            neighbours_to_connect = min(self.MAX_OUTBOUND_CONNECTIONS - len(self.outbound), \
    ...
    def _outdated_connections(self, t):
        ...
        for address, timestamp in self.outbound.items():
            if self.addrMan[address] + self.ADDRMAN_REPLACEMENT_HOURS * 60 * 60 < timestamp:
                print("found outdated connection")
                envelope = self._get_empty_envelope(t, address)
    ...
    def _get_address_to_connect(self):
        if len(self.outbound) == 0 or len(self.record_outgoing_conn['RandomNodes']) <= math.floor(self.MAX_OUTBOUND_CONNECTIONS * 0.5):
    ...
    def _insert_into_bucket(self, node_id):
        ...
        if node_id in self.bucket[bucket]:
            return
        if len(self.bucket[bucket]) >= 16:
            #check if the last entry in the bucket is online
            last_entry = self.bucket[bucket].pop()
    ...
    def _add_to_lookupBuf(self, sender_node_id, sender_rnd_id):
        while len(self.lookupBuf) >= 15:
            remove_key = list(self.lookupBuf.keys()).pop()
