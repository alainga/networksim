...
class Simulation:
    ...
    def __init__(self, simulation_type='bitcoin_protocol', MAX_OUTBOUND_CONNECTIONS=8, with_evil_nodes=False,
                 ...
                 outbound_distribution='const8_125', data={}, ti=0, lite=False):  # bitcoin_protocol power_2_choices
        ...
        for ii, t in enumerate(list(np.linspace(t_start, t_end, n_iterations))):
            ...
            if finish_simulation_counter == 0:
                ...
                for node_id in random.choices(list(self.DG.nodes), k=number_of_dying_nodes):
                    if node_id not in self.FIXED_DNS and node_id in list(self.DG.nodes):
                ...
                if finish_simulation_counter > finish_simulation_counter_max:
                    #for n in range (stop_growing, len(self.DG.nodes) + 1):
                     #   self.DG.remove_node(n)  # this is necessary for comparison reasons, maybe not the best solution
    ...
    def _process_envelopes(self, node_id):
        ...
        for envelope in envelopes:
            ...
            # check for unsolicited addr message
            if 10 >= len(envelope['address_list']) > 0:  # todo
                self._whispering(envelope['sender'])


    def _whispering(self, id):  # added by A. assuming graph is connected!
        for node_id in self.DG.nodes:
            node = self.all_nodes[node_id]
            if id not in node.addrMan and node_id != id:
                node.addrMan[id] = self.simulation_time
    ...
    def _new_node_connects_to_network(self, show_network=False):
        ...
        if self.simulation_protocol == 'bitcoin_protocol':
            ...
            node = bn.BitcoinNode(self.DG_last_id,
                                  self.simulation_time,
                                  self.FIXED_DNS,
                                  self.DG,
        ...
        # alain trying out some stuff
        # if self.DG_last_id == 1000:
        #   node = self.DG.nodes[self.DG_last_id][self.simulation_protocol]
        #  self.hack_it(node)
    ...
    def hack_it(self, node):
        print("now in hacking mode\n ! \n ! \n !")
        envelope = node.ask_neighbour_to_get_addresses(self.simulation_time)
        print("got envelope: ", envelope)
        print("leaving hacking mode\n ! \n ! \n !")
    ...
    def _initialize_fixed_dns_servers(self):
        for ii in self.FIXED_DNS:
            ...
            self.DG.add_node(ii)
            if self.simulation_protocol == 'bitcoin_protocol':
                node = bn.BitcoinNode(ii, self.simulation_time, self.FIXED_DNS, self.DG,
                                      connection_strategy=self.connection_strategy)
                self.DG.nodes[ii][self.simulation_protocol] = node
                self.all_nodes[ii] = node
            elif self.simulation_protocol == 'power_2_choices':
...
if __name__ == '__main__':
    ...
    stop_growing = 300  # network stops growing if reached this amount of nodes
