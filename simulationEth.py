...
import EthereumNode as en
import TheoryEthereumNode as ten
...
class Simulation:
    ...
    def __init__(self, simulation_type='ethereum_protocol', MAX_OUTBOUND_CONNECTIONS=13, with_evil_nodes=False,
                 ...
                 outbound_distribution='const13_25', data={}, ti=0, lite=False):
        ...
        self.all_nodes = dict()
        self.lite = lite  # not used anymore
    ...
    def run(self, t_start=1, t_end=60, n_iterations=124, plot_first_x_graphs=10,
            ...
            MAX_OUTBOUND_CONNECTIONS=13, numb_nodes=6000):
        self.MAX_OUTBOUND_CONNECTIONS = MAX_OUTBOUND_CONNECTIONS
        max_growth_rate = 0.05
        max_reconnect_rate = 0.9
        max_die_rate = 0.03
        finish_simulation_counter_max = 100
        ...
        for ii, t in enumerate(list(np.linspace(t_start, t_end, n_iterations))):
            ...
            #print(finish_simulation_counter)
            if finish_simulation_counter == 0:
                ...
                for node_id in random.choices(list(self.DG.nodes), k=number_of_dying_nodes):
                    ...
                    self.numb_went_off += 1

                number_of_new_nodes = random.randint(1, ceil(max_growth_rate * len(self.DG.nodes)))
                ...
                if finish_simulation_counter >= finish_simulation_counter_max:
                    if self.outbound_distribution == 'hacky_1':
                        self._hacky_1()
                    # self._check_in_out_graph()
                    #print(self.DG.degree())
                    print(self.numb_went_off, " nodes went offline and ", self.numb_went_on, " went online again")
            ...
            if (ii in avg_paths_after_n_iterations) and (finish_simulation_counter == 0):
                # self.whiteboard.avg_path_lengath_log()
                self.whiteboard.plot_degree()
                self.whiteboard.shortest_path_histogram_undirected()

            if (len(self.DG.nodes) >= numb_nodes) and (finish_simulation_counter == 0):  # intitiates termination
                finish_simulation_counter += 1
    ...
    def _hacky_1(self) -> bool:
        ...
        while True:
            ...
            for node_id in self.DG.nodes():
                self._process_envelopes(node_id)
                outbound_is_full = outbound_is_full and self.DG.nodes[node_id][self.simulation_protocol].outbound_is_full()
            if outbound_is_full:
                return True
    ...
    def _process_envelopes(self, node_id):
        ...
        for envelope in envelopes:
            ...
            if envelope['receiver'] not in self.DG.nodes:
                if envelope['receiver'] in self.DG.nodes[node_id][self.simulation_protocol].record_outgoing_conn[
                    'RandomNodes']:
                    self.DG.nodes[node_id][self.simulation_protocol].record_outgoing_conn['RandomNodes'].remove(
                        envelope['receiver'])
                if envelope['receiver'] in self.DG.nodes[node_id][self.simulation_protocol].record_outgoing_conn[
                    'lookupBuf']:
                    self.DG.nodes[node_id][self.simulation_protocol].record_outgoing_conn['lookupBuf'].remove(
                        envelope['receiver'])
    ...
    def _new_node_connects_to_network(self, show_network=False):
        ...
        if self.simulation_protocol == 'ethereum_protocol':
            ...
            if not self.lite:
                node = en.EthereumNode(self.DG_last_id,
                                       self.node_id_to_rnd,
                                       self.simulation_time,
                                       self.FIXED_DNS,
                                       self.DG,
                ...
                node = ten.TheoryEthereumNode(self.DG_last_id,
                                       self.node_id_to_rnd,
                                       self.simulation_time,
                                       self.FIXED_DNS,
                                       self.DG,
                                       ...
                                       connection_strategy=self.connection_strategy,
                                       MAX_TOTAL_CONNECTIONS=max_total_connections)
            self.DG.nodes[self.DG_last_id][self.simulation_protocol] = node
            self.all_nodes[self.DG_last_id] = node
            if False:
                self.DG.nodes[self.DG_last_id-5][self.simulation_protocol].connect_lite(self.simulation_time)
                self.DG.nodes[self.DG_last_id - 4][self.simulation_protocol].connect_lite(self.simulation_time)
                self.DG.nodes[self.DG_last_id - 3][self.simulation_protocol].connect_lite(self.simulation_time)
                self.DG.nodes[self.DG_last_id - 2][self.simulation_protocol].connect_lite(self.simulation_time)
                self.DG.nodes[self.DG_last_id - 1][self.simulation_protocol].connect_lite(self.simulation_time)
            if is_evil:
    ...
    def _node_ethereum(self, envelope_1, node_id, show_protocol, show_connection_failures):  # gets called if 2 nodes try to connect
        # if the node to connect does not exist anymore
        if envelope_1['receiver'] not in self.DG.nodes:
            if show_connection_failures:
        ...
        if envelope_3['connect_as_outbound'] == 'done':
            ...
            #print("added edge between ", list(envelope_1['sender'].keys()).pop(), " and ", envelope_1['receiver'])
        elif show_protocol:
            print(str(node_id) + ' could not update its outbound connections')
    ...
    def _initialize(self):  # initializes fixed dns servers and complete network of 13 nodes
        ...
        for ii in self.FIXED_DNS:
            ...
            self.DG.add_node(ii)
            if self.simulation_protocol == 'ethereum_protocol':
                self.DG.nodes[ii][self.simulation_protocol] = en.EthereumNode(ii, self.node_id_to_rnd,
                                                                              self.simulation_time, self.FIXED_DNS,
                                                                              self.DG,
                                                                              connection_strategy=self.connection_strategy)
            elif self.simulation_protocol == 'power_2_choices':
                self.DG.nodes[ii][self.simulation_protocol] = p2c.Power2Choices(ii, self.simulation_time,
                                                                                self.FIXED_DNS)
    ...
    def check_in_out_graph(self):
        num25 = 0
        nomatch = 0
        for n in self.DG.nodes:
            node = self.DG.nodes[n][self.simulation_protocol]
            deg = self.DG.degree(n)
            if deg > 25:
                print("node ", n, " has degree ", self.DG.degree(n))
            if deg == 25:
                num25 += 1
            if deg != len(node.inbound) + len(node.outbound):
                peers = list()
                nomatch += 1
                print("node ", n, "'s connections: deg in graph=", deg, ", in=", len(node.inbound), ", out=",
                      len(node.outbound))
                for i in list(node.inbound.keys()) + list(node.outbound.keys()):
                    peers.append((i))
                    if not self.DG.has_edge(n, i):
                        print("edge between ", n, " and ", i, " does not exist in graph!")
                print("peers of node ", n, ": ", peers)
        print(nomatch, " node connections don't match!")
        print("there are ", num25, " nodes with degree 25 ")
...
if __name__ == "__main__":
    ...
    w = s.whiteboard
    #w.plot_degree()
    w.plot_degree_in_time(plotsolo=True)
    #w.degree_in_time_Ethereum()  # interpolated
    #w.plot_degree2()
    # w.betweenness()
    w.clustering_in_time(plotsolo=True)
    w.shortest_path_histogram_undirected(plotsolo=True, is_final=True)
    w.closeness_in_time(plotsolo=True)
